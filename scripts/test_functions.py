...
def _pg_void_call(pg_conn, sql: str, args: tuple, ok_label: str):
    """Execute a void PG function inside a transaction; rollback; return (result_str, ok_bool)."""
    cur = pg_conn.cursor()
    try:
        cur.execute(sql, args)
        _rollback_safe(pg_conn)
        return ok_label, True
...
def run_cat_A(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if ms_id is None and pg_id is None:
        return "SKIP (table empty)", "SKIP (table empty)", True, "both tables empty"
    ...
    if pair.mssql_file is None:
        ...
        return "SKIP (no MSSQL SP)", pg_result, ok, "PG-only, no MSSQL counterpart"
    ...
    if ms_err and pg_err:
        ...
        return ms_result, pg_result, True, "Both raised error (consistent)"
...
def run_cat_C(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if ms_id is None and pg_id is None:
        return "SKIP (empty)", "SKIP (empty)", True, "both tables empty"
    ...
    if pair.mssql_file is None:
        ...
        return ms_result, pg_result, pg_ok, "PG-only, no MSSQL counterpart"
    ms_err = "ERROR" in ms_result
    pg_err = "ERROR" in pg_result
    if ms_err and pg_err:
        return ms_result, pg_result, True, "Both raised error (consistent)"
    ...
    match = ms_ok and pg_ok
    return ms_result, pg_result, match, "" if match else "value mismatch"
...
def run_cat_D(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if pair.mssql_file is None:
        pg_err = "ERROR" in pg_result
        return ms_result, pg_result, not pg_err, "PG-only, no MSSQL counterpart"
    ...
    if "ERROR" in ms_result or "ERROR" in pg_result:
        ...
        if pair.pg_schema == "integration" and "_archive" in pg_result.lower():
            return ms_result, pg_result, False, "PG archive tables not migrated"
        return ms_result, pg_result, False, "One side errored"
    ...
    if pair.pg_name == "search_for_stock_items" and pair.pg_schema == "webapi":
        return _compare_search_json(ms_rows, pg_rows)
    ...
    if pair.pg_schema == "website" and pair.pg_return_type == "table":
        return _compare_website_json_vs_table(ms_rows, pg_rows, pair)
...
def run_cat_E(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if pair.mssql_file is None:
        ...
        return ms_result, pg_result, ok, "PG-only, no MSSQL counterpart"
...
def run_cat_F(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if pair.pg_name == "activate_website_logon":
        ...
        return "SKIP", "SKIP", True, "unknown auth function"
...
def run_cat_G_complex(ms_conn, pg_conn, pair: FunctionPair):
    ...
    def _ret_pg(pg_res, ok, ms_label=PG_ONLY[0], note=PG_ONLY[1]):
        return ms_label, pg_res, ok, note

    # ── WebApi inserts ────────────────────────────────────────────────────────

    if fn == "insert_state_provinces_from_json":
        if not f.get("country_id"):
            return PG_ONLY[0], "SKIP (no country data)", True, "no countries in DB"
        jdata = json.dumps([{
        ...
        pg_res, ok = _pg_void_call(pg_conn, f"SELECT {sch}.{fn}(%s,%s)", (jdata, 1), "inserted OK (count=1)")
        return _ret_pg(pg_res, ok)

    if fn == "insert_cities_from_json":
        if not f.get("state_province_id"):
            return PG_ONLY[0], "SKIP (no state province data)", True, "no state provinces in DB"
        jdata = json.dumps([{
    ...
    if fn == "insert_purchase_orders_from_json":
        if not all(f.get(k) for k in ("supplier_id", "delivery_method_id", "person_id")):
            return PG_ONLY[0], "SKIP (missing FK data)", True, "no supplier/delivery/person data"
        jdata = json.dumps([{
    ...
    if fn == "insert_purchase_order_lines_from_json":
        if not all(f.get(k) for k in ("purchase_order_id", "stock_item_id", "package_type_id")):
            return PG_ONLY[0], "SKIP (missing FK data)", True, "no PO/item/package data"
        jdata = json.dumps([{
    ...
    if fn == "insert_customers_from_json":
        ...
        if not all(f.get(k) for k in need):
            return PG_ONLY[0], "SKIP (missing FK data)", True, "missing required FK data"
        jdata = json.dumps([{
    ...
    if fn == "insert_stock_items_from_json":
        if not all(f.get(k) for k in ("supplier_id", "color_id", "package_type_id")):
            return PG_ONLY[0], "SKIP (missing FK data)", True, "no supplier/color/package data"
        jdata = json.dumps([{
    ...
    if fn == "update_customer_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no customer data)", True, "no customers in DB"
    ...
    if fn == "update_customer_transaction_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no customer transaction data)", True, "empty table"
    ...
    if fn == "update_invoice_from_json":
        pk = f.get("invoice_id")
        if not pk:
            return PG_ONLY[0], "SKIP (no invoice data)", True, "empty table"
    ...
    if fn == "update_purchase_order_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no purchase order data)", True, "empty table"
    ...
    if fn == "update_purchase_order_line_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no PO line data)", True, "empty table"
    ...
    if fn == "update_sales_order_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no order data)", True, "empty table"
    ...
    if fn == "update_special_deal_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no special deal data)", True, "empty table"
    ...
    if fn == "update_stock_item_from_json":
        ...
        if not row:
            return PG_ONLY[0], "SKIP (no stock item data)", True, "empty table"
        si_id, color_id = row
        jdata = json.dumps({
    ...
    if fn == "update_supplier_from_json":
        ...
        if not pk or not dm:
            return PG_ONLY[0], "SKIP (missing FK data)", True, "no supplier/delivery data"
        jdata = json.dumps({
            "DeliveryMethodID":    dm,
            "SupplierReference":   f"REF{uid[:8]}",
            "BankInternationalCode": "TSTBNK",
    ...
    if fn == "update_supplier_transaction_from_json":
        ...
        if not pk:
            return PG_ONLY[0], "SKIP (no supplier transaction data)", True, "empty table"
        jdata = json.dumps({
    ...
    if fn == "record_cold_room_temperatures":
        ...
        pg_res, ok = _pg_void_call(pg_conn, f"SELECT {sch}.{fn}(%s::jsonb)", (jdata,), "executed OK")
        return _ret_pg(pg_res, ok, TVP[0], TVP[1])
    ...
    if fn == "insert_customer_orders":
        ...
        if not all([cust, person, si]):
            return TVP[0], "SKIP (missing FK data)", True, "missing customer/person/item data"
    ...
    if fn == "invoice_customer_orders":
        ...
        try:
            ...
            if not row:
                _rollback_safe(pg_conn)
                return TVP[0], "SKIP (insert_customer_orders created no order)", True, "setup failed"
            order_id = row[0]
        ...
        return _ret_pg(pg_res, ok, TVP[0], "MSSQL uses TVP; PG-only test (created+picked+invoiced in txn)")

    # Unknown G_complex function — should not happen with full dispatch above
    return "SKIP", "SKIP", True, "G_complex — fixture not implemented for this function"
...
def run_cat_H(ms_conn, pg_conn, pair: FunctionPair):
    ...
    if "ERROR" in ms_result or "ERROR" in pg_result:
        ms_err = "ERROR" in ms_result
        pg_err = "ERROR" in pg_result
        return ms_result, pg_result, ms_err == pg_err, "both errored" if (ms_err and pg_err) else "one side errored"
...
def run_pair(ms_conn, pg_conn, pair: FunctionPair):
    ...
    try:
        ...
        return "HARNESS ERROR", f"HARNESS ERROR: {str(exc)[:100]}", False, str(exc)[:100]
...
def write_report(results_by_schema: dict, elapsed: float) -> str:
    ...
    for schema_key in ("WebApi", "Integration", "Website"):
        rows = results_by_schema.get(schema_key)
        if not rows:
            continue
        ...
        for pair, ms_res, pg_res, match, notes in rows:
            if match:
                icon = "✓"
            elif "archive tables not migrated" in notes.lower():
                icon = "⚠"
            else:
                icon = "✗"
...
def main():
    ...
    for pair in pairs:
        ...
        ms_res, pg_res, match, notes = run_pair(ms_conn, pg_conn, pair)
        ...
        results_by_schema.setdefault(schema_key, []).append(
            (pair, ms_res, pg_res, match, notes))
