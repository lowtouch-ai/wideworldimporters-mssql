...
CREATE SEQUENCE IF NOT EXISTS sequences.stock_group_id_seq START 1 INCREMENT 1;
...
COMMENT ON TABLE warehouse.stock_groups IS 'Groups for categorizing stock items (ie: novelties, toys, edible novelties, etc.)';
COMMENT ON COLUMN warehouse.stock_groups.StockGroupID IS 'Numeric ID used for reference to a stock group within the database';
COMMENT ON COLUMN warehouse.stock_groups.StockGroupName IS 'Full name of groups used to categorize stock items';
