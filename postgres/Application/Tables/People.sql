...
CREATE TABLE application.people (
    ...
    SearchName              TEXT            GENERATED ALWAYS AS (PreferredName || ' ' || FullName) STORED NOT NULL,
    ...
    OtherLanguages          TEXT            NULL, -- NOTE: was a non-persisted computed column (json_query on CustomFields.OtherLanguages)
...
CREATE INDEX IX_Application_People_IsEmployee ON application.people (IsEmployee ASC);
CREATE INDEX IX_Application_People_IsSalesperson ON application.people (IsSalesperson ASC);
CREATE INDEX IX_Application_People_FullName ON application.people (FullName ASC);
CREATE INDEX IX_Application_People_Perf_20160301_05 ON application.people (IsPermittedToLogon ASC, PersonID ASC) INCLUDE (FullName, EmailAddress);

-- 4 index-level extended properties omitted (index comments not supported via standard DDL)
