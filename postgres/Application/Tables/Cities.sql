...
CREATE TABLE application.cities (
    ...
    CONSTRAINT FK_Application_Cities_StateProvinceID_Application_StateProvinces FOREIGN KEY (StateProvinceID) REFERENCES application.state_provinces (StateProvinceID)
);

CREATE INDEX FK_Application_Cities_StateProvinceID ON application.cities (StateProvinceID ASC);

-- 1 index-level extended property omitted (index comments not supported via standard DDL)
