...
-- 4 index-level extended properties omitted (no PostgreSQL equivalent for index comments)
CREATE INDEX FK_Sales_Orders_CustomerID ON sales.orders (CustomerID ASC);
CREATE INDEX FK_Sales_Orders_SalespersonPersonID ON sales.orders (SalespersonPersonID ASC);
CREATE INDEX FK_Sales_Orders_PickedByPersonID ON sales.orders (PickedByPersonID ASC);
CREATE INDEX FK_Sales_Orders_ContactPersonID ON sales.orders (ContactPersonID ASC);
