...
-- ConfirmedDeliveryTime and ConfirmedReceivedBy were non-persisted computed columns in MSSQL.
-- Converted to regular nullable columns; populate via trigger or application logic if needed.
...
(
    ...
    ConfirmedDeliveryTime       TIMESTAMP(6)   NULL, -- NOTE: was a non-persisted computed column
    ConfirmedReceivedBy         TEXT           NULL, -- NOTE: was a non-persisted computed column
...
-- 9 index-level extended properties omitted (no PostgreSQL equivalent for index comments)
