...
CREATE OR REPLACE FUNCTION webapi.insert_state_provinces_from_json(
    ...
    p_user_id         integer
) RETURNS TABLE(stateprovinceid integer) AS $$
BEGIN
    RETURN QUERY
    INSERT INTO application.state_provinces(
        stateprovincecode, stateprovincename, countryid, salesterritory,
        latestrecordedpopulation, lasteditedby
    )
    SELECT
        x."StateProvinceCode", x."StateProvinceName", x."CountryID", x."SalesTerritory",
        x."LatestRecordedPopulation", p_user_id
    ...
    RETURNING state_provinces.stateprovinceid;
END;
$$ LANGUAGE plpgsql;
