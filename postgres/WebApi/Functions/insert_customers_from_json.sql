...
CREATE OR REPLACE FUNCTION webapi.insert_customers_from_json(
    p_customers text,
    p_user_id   integer
) RETURNS TABLE(customerid integer) AS $$
BEGIN
    RETURN QUERY
    ...
    SELECT
        ...
        x."StandardDiscountPercentage", x."IsStatementSent", x."IsOnCreditHold", x."PaymentDays",
        x."PhoneNumber", x."FaxNumber", x."DeliveryRun", x."RunPosition", x."WebsiteURL",
        x."DeliveryAddressLine1", x."DeliveryAddressLine2", x."DeliveryPostalCode",
    ...
    RETURNING customers.customerid;
END;
$$ LANGUAGE plpgsql;
