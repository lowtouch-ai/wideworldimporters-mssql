...
FROM purchasing.suppliertransactions AS st
    ...
    LEFT JOIN application.transactiontypes AS tt ON st.TransactionTypeID = tt.TransactionTypeID
    LEFT JOIN purchasing.suppliers AS s ON st.SupplierID = s.SupplierID
    LEFT JOIN application.paymentmethods AS pm ON st.PaymentMethodID = pm.PaymentMethodID;
