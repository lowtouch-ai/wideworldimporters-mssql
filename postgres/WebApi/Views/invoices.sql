...
FROM sales.invoices AS inv
    INNER JOIN sales.customers AS c ON inv.CustomerID = c.CustomerID
    INNER JOIN application.deliverymethods AS dm ON inv.DeliveryMethodID = dm.DeliveryMethodID
    INNER JOIN application.people AS contact ON inv.ContactPersonID = contact.PersonID
    INNER JOIN application.people AS sp ON inv.SalespersonPersonID = sp.PersonID;
