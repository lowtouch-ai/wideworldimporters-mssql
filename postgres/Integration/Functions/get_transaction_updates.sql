...
BEGIN
    RETURN QUERY
    SELECT
        CAST(ct.TransactionDate AS date),
    ...
    LEFT JOIN sales.invoices AS i ON ct.InvoiceID = i.InvoiceID
    WHERE ct.LastEditedWhen > p_LastCutoff
      AND ct.LastEditedWhen <= p_NewCutoff
    ...
    WHERE st.LastEditedWhen > p_LastCutoff
      AND st.LastEditedWhen <= p_NewCutoff;
END;
$$ LANGUAGE plpgsql;
